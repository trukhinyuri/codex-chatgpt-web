...
3. **Ship only through `main`.** Push to `main` only commits that passed `bun run verify` in a clean clone. Installed launchers build `main` themselves, run the full suite, install only after Codex has been idle (10 minutes, or 30 seconds after a click on the update button), and roll back a build that does not report a healthy start within 6 minutes. Never run the installer script from a development branch on the computer you work on.
