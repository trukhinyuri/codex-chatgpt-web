...
5. Many users can share one pool of accounts (for example a team whose seats form the pool): turns from all users are scheduled fairly across the pool, per-account limits are respected across all users, and no single machine or service is a point of failure for the others. Each account is used within its provider's terms (a pool of individual seats, not one seat shared by many people).
