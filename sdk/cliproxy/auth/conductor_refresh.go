...
const (
	...
	//
	// This is a passive fallback that applies to every provider. Yuri's active
	// quota-recovery loop (quota_recovery.go, opt-in via
	// quota-recovery-interval-seconds) independently re-checks Codex and Claude
	// credentials on a much shorter interval (default 5m) and can shorten or lift a
	// cooldown sooner via ProbeQuota; this ceiling does not touch that mechanism and
	// only bounds the worst case for providers without a prober.
