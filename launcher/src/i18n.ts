...
const en = {
  ...
  launchAtLoginBodyMac: "Start at login and restart automatically if the launcher quits unexpectedly, so the local Responses route is ready before Codex opens.",
  keepRunningOnCloseBodyMac: "Closing the window keeps the launcher running; reopen it from the Dock. Native and ChatGPT Web models stay available.",
  launchAtLoginBlocked: "macOS keeps Codex Web GPT from running in the background, so it does not start at login or restart after a crash. Allow it under Login Items in System Settings.",
  showInMenuBar: "Show in menu bar",
  showInMenuBarBody: "Add an icon to the menu bar. Codex Web GPT stays in the Dock either way, and closing its window keeps the models available.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  launchAtLoginBodyMac: "登录时启动，启动器意外退出后自动重新启动，让本地 Responses 路由在 Codex 打开前就已可用。",
  keepRunningOnCloseBodyMac: "关闭窗口后启动器继续运行，可从程序坞重新打开。原生模型和 ChatGPT Web 模型保持可用。",
  launchAtLoginBlocked: "macOS 不允许 Codex Web GPT 在后台运行，因此它不会在登录时启动，也不会在崩溃后重新启动。请在“系统设置”的“登录项”中允许它。",
  showInMenuBar: "在菜单栏中显示",
  showInMenuBarBody: "在菜单栏中添加图标。无论是否开启，Codex Web GPT 都会保留在程序坞中；关闭窗口后模型仍然可用。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  launchAtLoginBodyMac: "ログイン時に起動し、ランチャーが予期せず終了した場合は自動的に再起動します。Codex を開く前からローカル Responses ルートを利用できます。",
  keepRunningOnCloseBodyMac: "ウィンドウを閉じてもランチャーは実行を続け、Dock から再び開けます。ネイティブモデルと ChatGPT Web モデルは引き続き利用できます。",
  launchAtLoginBlocked: "macOS が Codex Web GPT のバックグラウンド実行を許可していないため、ログイン時に起動せず、クラッシュ後も再起動しません。システム設定の「ログイン項目」で許可してください。",
  showInMenuBar: "メニューバーに表示",
  showInMenuBarBody: "メニューバーにアイコンを追加します。この設定にかかわらず Codex Web GPT は Dock に表示され、ウィンドウを閉じてもモデルは引き続き利用できます。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  launchAtLoginBodyMac: "로그인 시 실행하고 런처가 예기치 않게 종료되면 자동으로 다시 시작하여, Codex를 열기 전부터 로컬 Responses 경로를 사용할 수 있게 합니다.",
  keepRunningOnCloseBodyMac: "창을 닫아도 런처는 계속 실행되며 Dock에서 다시 열 수 있습니다. 네이티브 모델과 ChatGPT Web 모델을 계속 사용할 수 있습니다.",
  launchAtLoginBlocked: "macOS가 Codex Web GPT의 백그라운드 실행을 허용하지 않아 로그인 시 실행되지 않고 충돌 후에도 다시 시작되지 않습니다. 시스템 설정의 로그인 항목에서 허용하세요.",
  showInMenuBar: "메뉴 막대에 표시",
  showInMenuBarBody: "메뉴 막대에 아이콘을 추가합니다. 이 설정과 관계없이 Codex Web GPT는 Dock에 표시되며, 창을 닫아도 모델을 계속 사용할 수 있습니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  launchAtLoginBodyMac: "登入時啟動，啟動器意外結束後會自動重新啟動，讓本機 Responses 路由在 Codex 開啟前就已可用。",
  keepRunningOnCloseBodyMac: "關閉視窗後啟動器會繼續執行，可從 Dock 重新開啟。原生模型與 ChatGPT Web 模型保持可用。",
  launchAtLoginBlocked: "macOS 不允許 Codex Web GPT 在背景執行，因此它不會在登入時啟動，也不會在當機後重新啟動。請在「系統設定」的「登入項目」中允許它。",
  showInMenuBar: "在選單列中顯示",
  showInMenuBarBody: "在選單列中加入圖示。無論是否開啟，Codex Web GPT 都會保留在 Dock 中；關閉視窗後模型仍可使用。",
