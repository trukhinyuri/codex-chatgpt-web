...
const en = {
  ...
  automaticUpdatesBody: "Install new versions of this fork by itself: each one is built from main, must pass every test and installs only after Codex has been idle for five minutes. If the new version does not start cleanly, the previous one is restored.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  automaticUpdatesBody: "自动安装此分支的新版本：每个版本都从 main 构建，必须通过全部测试，并且只在 Codex 空闲五分钟后安装。如果新版本未能正常启动，将恢复上一个版本。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  automaticUpdatesBody: "このフォークの新しいバージョンを自動でインストールします。各バージョンは main からビルドされ、すべてのテストに合格する必要があり、Codex が 5 分間アイドルになった後にのみインストールされます。新しいバージョンが正常に起動しない場合は、以前のバージョンに戻します。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  automaticUpdatesBody: "이 포크의 새 버전을 자동으로 설치합니다. 각 버전은 main에서 빌드되고 모든 테스트를 통과해야 하며, Codex가 5분 동안 유휴 상태일 때만 설치됩니다. 새 버전이 정상적으로 시작되지 않으면 이전 버전으로 복원합니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  automaticUpdatesBody: "自動安裝此分支的新版本：每個版本都從 main 建置，必須通過所有測試，且只在 Codex 閒置五分鐘後安裝。如果新版本無法正常啟動，會還原為上一個版本。",
