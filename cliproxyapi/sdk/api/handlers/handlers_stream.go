...
type sseJSONValidationState struct {
	...
	pendingErr     error
	prevEndsWithCR bool
	// forwarded marks that at least one complete frame (or a validated
	// trailing payload) has already been written to output for this stream.
	// Once true, a later whitespace-only chunk may hold nothing but the
	// remaining line-ending bytes of the delimiter that closed the previous
	// frame (e.g. a CRLF split so the trailing "\n" lands in its own
	// AddChunk call) and must still be forwarded, not silently dropped.
	forwarded bool
}
...
func (s *sseJSONValidationState) AddChunk(chunk []byte) ([]byte, error) {
	...
	if len(bytes.TrimSpace(s.pending)) == 0 {
		// A chunk that is only whitespace can still be real output once a
		// frame has already been forwarded: it may be the remainder of the
		// blank-line delimiter that a split CRLF pushed into its own chunk.
		if s.forwarded {
