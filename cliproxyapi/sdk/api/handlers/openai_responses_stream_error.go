...
//
// The two are kept together deliberately. They are halves of one
// classification and a client reads both: the code names the condition, the
// type says whose fault it was and therefore whether a retry is allowed.
// Deriving them in two separate places (a status switch for the code, a
// coarse >=500 threshold for the type) let them contradict each other: a
// Codex stream cut before response.completed is reported as HTTP 408 with
// the retryable code "request_timeout" alongside the type
// "invalid_request_error" — a type that tells every downstream consumer the
// request was malformed and must not be replayed. See upstream
// router-for-me/CLIProxyAPI issue #5931 / PR #5932.
...
//
// StatusRequestTimeout is the reason this table exists: a Codex stream that
// ends before response.completed is surfaced with that status, and the
// request itself was well formed — the connection failed, so the caller may
// replay it.
