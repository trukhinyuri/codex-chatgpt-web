...
// codexUsageLimitMaxCooldown caps the cooldown derived from a usage_limit_reached
// body. Codex reports resets_at/resets_in_seconds for whichever quota window the
// backend chooses to echo, and that is not always the window actually exhausted:
// production evidence showed a 429 body naming the multi-day secondary window
// (~68h) while only the ~5h primary window was exhausted, so the credential was
// scheduled for a multi-day cooldown that cleared on its own hours later. Capping
// at 6h (comfortably above the ~5h primary window) bounds the cost of a wrong
// body-reported deadline to one extra upstream request instead of a multi-day
// stall; Yuri's ProbeQuota-based quota recovery loop (codex_quota_probe.go)
// independently re-checks and can shorten a cooldown sooner, but that probe is
// opt-in per deployment, so this cap is a bound that never regresses a genuinely
// short cooldown.
